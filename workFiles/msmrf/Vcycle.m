function [L, prior, Q] = Vcycle(U,E,P,L,prior,gP,fineGraph,nDepth)

%
% msmrf(D,E,W,L,gP)    -   A multiscale scheme for finding minimum energy
...
% for CompCoarseGraph()
if gP.bAggMode
    vEdgeList = sortEdges(U,E,P,prior,L,gP);
else
...
if (size(Uc,1)/size(U,1) >= gP.crsRatioThrs) || ...
        (size(Uc,1) <= 500)
    % coarsening ratio is above threshold,
    % stop the recursion
...
%
% recursive call
[Lc, priorc, Qc] = Vcycle(Uc,Ec,Pc,Lc,priorc,gP,false,nDepth+1);
Q.e = [Q.e, Qc.e];
Q.nv = [Q.nv, Qc.nv];
