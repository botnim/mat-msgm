...
if gP.b2lbl
    ...
    vidxInf = any(any(isinf(P),1),2);  	% indices of edges with Infs
end
...
for iM = 1 : size(vEdgeList,1)
    ...
    %
    % when working kto2 framework, another rule to avoid "inf-spreading"
    % if i_k1 -> j_k2 then i_k,j_k cannot be interpolated for all k
    if gP.b2lbl && ~vidxInf(abs(pp)) % && any(vidxInf)
        % about to contract non-Inf edge
    ...
    if (vFine(ii) == 0) && (vFine(jj) == 0)
        ...
        % DEBUG DEBUG
        pairwise = bsxfun(@plus,pairwise,U(ii,:));
        % DEBUG DEBUG
...
%             if any(U(ii,sum(mbMin,1) > 1) == min(U(ii,:)))
