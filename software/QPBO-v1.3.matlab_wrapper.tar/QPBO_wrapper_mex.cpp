 ...
 *
 * compiling using:
 * >> mex -O -largeArrayDims CXXFLAGS="\$CXXFLAGS -Wno-write-strings" QPBO.cpp QPBO_extra.cpp QPBO_maxflow.cpp QPBO_postprocessing.cpp QPBO_wrapper_mex.cpp -output QPBO_wrapper_mex
 */
    ...
    const mxArray* pin[])
{
    ...
    if ( mxGetNumberOfDimensions(pin[iU]) != 2 || mxGetM(pin[iU]) != 2 )
        mexErrMsgIdAndTxt("QPBO_wrapper_mex:unary_term_size","Unary term must be 2xN matrix");
