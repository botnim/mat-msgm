...
public:
	...
	unsigned int DumpCurrentState(int* mIds, double* messages);
	
	void HotStart(const NodeId* mIds, const double* messages, unsigned int nm);

	REAL PrimalInitTRWS(const NodeId* mIds, const Label* primal_sol);
	REAL PrimalInit(const NodeId* mIds, const Label* primal_sol);
	void SetSol(NodeId i, Label k) { i->m_solution = k; }
...
// bagon moved to public...	REAL ComputeSolutionAndEnergy(); // sets Node::m_solution, returns value of the energy
